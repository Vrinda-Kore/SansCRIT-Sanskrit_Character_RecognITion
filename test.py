...
for filename in os.listdir(input_folder):
    if filename.endswith('.jpg'):  # Filter image files (adjust file extension if needed)
        # Reading the image in grayscale
        img = cv2.imread(os.path.join(input_folder, filename), 0)

        # Threshold the grayscale image to create a binary image
        _, blackAndWhite = cv2.threshold(img, 220, 255, cv2.THRESH_BINARY_INV)

        # Identify connected components and their statistics
        nlabels, labels, stats, centroids = cv2.connectedComponentsWithStats(blackAndWhite, None, None, None, 8, cv2.CV_32S)
        sizes = stats[1:, -1]  # get CC_STAT_AREA component
        img2 = np.zeros(labels.shape, np.uint8)
        ...
        # Invert the filtered image to get the result
        res = cv2.bitwise_not(img2)
